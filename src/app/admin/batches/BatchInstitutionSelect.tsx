...
import { fetchInstitutionsLookup } from "@/features/admin/institutions/api/institution-api";
...
export default function BatchInstitutionSelect({ value, onChange, initialName, error }: InstitutionSelectProps) {
  ...
  const { data: institutions, isLoading } = useQuery<any[], Error>({
    queryKey: ["institutionsLookup", debouncedSearch],
    queryFn: () => fetchInstitutionsLookup(debouncedSearch),
    staleTime: 5 * 60 * 1000,
  ...
  useEffect(() => {
    if (value && institutions) {
      const match = institutions.find((inst: any) => String(inst.id) === String(value));
      if (match) {

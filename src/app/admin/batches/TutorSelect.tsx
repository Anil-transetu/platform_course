...
export default function TutorSelect({ value, onChange, initialName, error }: TutorSelectProps) {
  ...
  // Update selectedName when tutors load and value matches
  useEffect(() => {
    if (value && tutors.length > 0) {
      const match = tutors.find((t: any) => String(t.id) === String(value));
      if (match) {
  ...
  const handleSelect = (tutorId: string, tutorName: string) => {
    onChange(tutorId);
    setSelectedName(tutorName);
    setIsOpen(false);
    setSearchTerm("");
  };
      ...
      >
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <span className={`truncate ${selectedName ? "text-gray-900 dark:text-foreground" : "text-gray-400"}`}>
            {selectedName || "Search and select instructor..."}
          </span>
        </div>
              ...
              <ul className="space-y-0.5">
                {tutors.map((tutor: any) => (
                  ...
                  >
                    ...
                    {String(value) === String(tutor.id) && <Check size={14} className="shrink-0" />}
                  </li>
                ))}
              </ul>
            ) : (
